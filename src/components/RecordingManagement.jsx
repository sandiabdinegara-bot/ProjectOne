...
import { AI_BASE_URL, IMAGE_BASE_URL } from '../config';
...
export default function RecordingManagement({ isHistory = false, user, canDo = () => true }) {
    const itemId = isHistory ? 'history_catat' : 'catat';
    // Data States
    ...
    const validateWithAI = async (imageFile, userInput) => {
        ...
        try {
            ...
            const response = await fetch(`${AI_BASE_URL}/validate`, {
                method: 'POST',
                body: formDataAi
            });
    ...
    const urlToFile = async (url, filename) => {
        try {
            const fullUrl = url.startsWith('/') ? `${IMAGE_BASE_URL}${url}` : url;
    ...
    return (
        <div style={{ animation: 'fadeIn 0.5s ease-out', display: 'flex', flexDirection: 'column', flex: 1, minHeight: 0 }}>
                            ...
                            <table className="customer-table" style={{ tableLayout: 'fixed', width: 'max-content', minWidth: '100%' }}>
                                <thead>
                                    <tr>
                                        ...
                                        {!isHistory && (canDo('catat', 'U') || canDo('catat', 'D')) && (
            ...
            <SimpleImageViewer
                ...
                imageSrc={previewImage && previewImage.startsWith('/') && !previewImage.startsWith('//') ? `${IMAGE_BASE_URL}${previewImage}` : previewImage}
