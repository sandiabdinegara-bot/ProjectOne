...
import { Search, Plus, Edit2, Trash2, X, Save, Download, FileSpreadsheet, FileText, Printer, SlidersHorizontal, ChevronLeft, ChevronRight, Map, CheckCircle, AlertCircle, Loader2 } from 'lucide-react';
import ConfirmModal from './ConfirmModal';
import { BASE_URL, DEFAULT_PAGE_SIZE, PAGE_SIZE_OPTIONS } from '../config';
import { fetchWithAuth } from '../api';

const API_URL = `${BASE_URL}/zone/kecamatan`;

// API shape: { id, kecamatan }
const ALL_COLUMNS = [
    { id: 'id', label: 'ID' },
    { id: 'kecamatan', label: 'KECAMATAN' }
];

const DEFAULT_COLUMNS = ['id', 'kecamatan'];
...
export default function KecamatanManagement() {
    ...
    const [limit, setLimit] = useState(() => {
        const saved = localStorage.getItem('pdam_kecamatan_page_size');
    ...
    const [formData, setFormData] = useState({ kecamatan: '' });

    const [isColumnMenuOpen, setIsColumnMenuOpen] = useState(false);
    const [visibleColumns, setVisibleColumns] = useState(() => {
        try {
            const saved = localStorage.getItem('pdam_visible_columns_kecamatan');
    ...
    useEffect(() => {
        localStorage.setItem('pdam_visible_columns_kecamatan', JSON.stringify(visibleColumns));
    }, [visibleColumns]);

    useEffect(() => {
        localStorage.setItem('pdam_kecamatan_page_size', String(limit));
    }, [limit]);
    ...
    const handleOpenModal = (item = null) => {
        if (item) {
            setEditingItem(item);
            setFormData({ kecamatan: item.kecamatan ?? '' });
        } else {
            setEditingItem(null);
            setFormData({ kecamatan: '' });
        }
        setIsModalOpen(true);
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const res = await fetchWithAuth(url, {
                ...
                body: JSON.stringify({ kecamatan: formData.kecamatan })
            });
            ...
            if (res.status === 201 || res.status === 200) {
                fetchItems(page, limit, searchTerm);
                handleCloseModal();
                showNotification(isUpdate ? 'Kecamatan berhasil diperbarui' : 'Kecamatan berhasil ditambahkan');
            } else if (data.error) {
                showNotification(data.error, 'error');
            }
        } catch (err) {
            console.error(err);
            showNotification('Gagal menyimpan data kecamatan', 'error');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleConfirmDelete = async () => {
        try {
            ...
            if (res.status === 404) {
                showNotification('Data kecamatan tidak ditemukan', 'error');
            ...
            if (res.status === 200) {
                ...
                showNotification(data.error || 'Gagal menghapus kecamatan', 'error');
            }
        } catch (err) {
    ...
    const exportToCSV = async () => {
        try {
            ...
            link.href = url;
            link.download = `data_kecamatan_${new Date().toISOString().split('T')[0]}.csv`;
            link.click();
    ...
    const exportToExcel = async () => {
        try {
            ...
            const link = document.createElement('a');
            link.href = url;
            link.download = `data_kecamatan_${new Date().toISOString().split('T')[0]}.xlsx`;
            link.click();
            URL.revokeObjectURL(url);
    ...
    const exportToPDF = async () => {
        try {
            ...
            const link = document.createElement('a');
            link.href = url;
            link.download = `data_kecamatan_${new Date().toISOString().split('T')[0]}.pdf`;
            link.click();
            URL.revokeObjectURL(url);
        ...
        <div style={{ animation: 'fadeIn 0.5s ease-out' }}>
            ...
            <style>
                {`
                    ...
                    .kecamatan-table { width: 100%; border-collapse: separate; border-spacing: 0; }
                    .kecamatan-table thead th { position: sticky; top: 0; z-index: 30; background: #f8fafc !important; box-shadow: inset 0 -2px 0 var(--border); text-transform: uppercase; font-size: 0.7rem; font-weight: 700; letter-spacing: 0.05em; color: #64748b; padding: 1rem 1.25rem !important; white-space: nowrap; }
                    .kecamatan-table tbody tr:nth-of-type(even) td { background-color: #fafbfc; }
                    .kecamatan-table tbody tr:hover td { background-color: #eff6ff !important; }
                    .kecamatan-table td, .kecamatan-table th { padding: 0.875rem 1.25rem !important; vertical-align: middle; font-size: 0.875rem; color: #334155; }
                    .sticky-col-left { position: sticky !important; left: 0; z-index: 20 !important; background: white !important; }
                    .kecamatan-table thead th.sticky-col-left { background: #f8fafc !important; z-index: 40 !important; box-shadow: inset 0 -2px 0 var(--border); }
                    .sticky-col-right { position: sticky !important; right: 0; z-index: 20 !important; background: white !important; }
                    .kecamatan-table thead th.sticky-col-right { background: #f8fafc !important; z-index: 40 !important; box-shadow: inset 0 -2px 0 var(--border); }
                    .kecamatan-table tbody tr:nth-of-type(even) .sticky-col-left, .kecamatan-table tbody tr:nth-of-type(even) .sticky-col-right { background-color: #fafbfc !important; }
                    .badge { padding: 0.25rem 0.75rem; border-radius: 9999px; font-size: 0.75rem; font-weight: 600; }
                    .badge-active { background-color: #dcfce7; color: #166534; }
                    .badge-inactive { background-color: #fee2e2; color: #991b1b; }
                    @media print { .no-print, .no-print * { display: none !important; } body { background: white !important; padding: 0 !important; margin: 0 !important; } .card { box-shadow: none !important; border: none !important; padding: 0 !important; margin: 0 !important; } .table-container { overflow: visible !important; max-height: none !important; width: 100% !important; padding: 0 !important; margin: 0 !important; } .kecamatan-table { width: 100% !important; border-collapse: collapse !important; table-layout: auto !important; font-size: 8pt !important; } .kecamatan-table th, .kecamatan-table td { border: 1px solid #e2e8f0 !important; padding: 4pt !important; overflow: visible !important; white-space: normal !important; word-break: break-word !important; min-width: 0 !important; width: auto !important; position: static !important; background: transparent !important; box-shadow: none !important; } .sticky-col-left { position: static !important; background: transparent !important; box-shadow: none !important; } @page { size: landscape; margin: 1cm; } header.header { margin-bottom: 1rem !important; } h1 { font-size: 16pt !important; } }
                `}
            </style>
            ...
            <header className="header" style={{ marginBottom: '2rem' }}>
                <div>
                    ...
                    <p style={{ color: '#64748b', fontSize: '0.95rem', marginTop: '0.25rem' }}>Kelola daftar kecamatan</p>
                </div>
                <div className="header-actions no-print" style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
                    ...
                    <button className="btn btn-primary" onClick={() => handleOpenModal()} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.625rem 1.25rem', borderRadius: '8px', fontSize: '0.95rem', fontWeight: 600, whiteSpace: 'nowrap', boxShadow: '0 4px 6px -1px rgba(14, 165, 233, 0.2)' }}>
                        <Plus size={20} /><span className="hide-mobile">Tambah Kecamatan</span>
                    </button>
                </div>
            </header>
            ...
            <div className="card">
                <div className="no-print" style={{ display: 'flex', gap: '0.75rem', marginBottom: '1.5rem', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap' }}>
                    <div style={{ display: 'flex', gap: '1rem', alignItems: 'center', flex: 1, minWidth: '300px' }}>
                        ...
                        <div style={{ position: 'relative', flex: '0 1 350px', display: 'flex', gap: '0.5rem' }}>
                            ...
                            <input style={{ paddingLeft: '2.5rem', paddingRight: '0.625rem', height: '42px', borderRadius: '8px', border: '1px solid var(--border)', fontSize: '0.9rem', width: '100%', background: '#f8fafc' }} placeholder="Cari nama kecamatan..." value={searchInput} onChange={(e) => setSearchInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && applySearch()} />
                ...
                <div className="table-container" style={{ maxHeight: 'calc(100vh - 350px)', overflowY: 'auto', margin: '1rem -1rem 0', width: 'calc(100% + 2rem)' }}>
                    <table className="kecamatan-table">
                        <thead>
                            <tr>
                                {displayedColumnIds.map(colId => {
                                    ...
                                    else style = { ...style, minWidth: '180px', textAlign: 'left' };
                        ...
                        <tbody>
                            {loading ? (
                                ...
                                <tr><td colSpan={displayedColumnIds.length + 1} style={{ padding: '3rem', textAlign: 'center', color: '#64748b' }}>{searchTerm ? `Tidak ditemukan hasil untuk "${searchTerm}"` : 'Tidak ada data kecamatan.'}</td></tr>
                            ) : (
                                items.map((item, index) => (
                                    <tr key={item.id}>
                                        {displayedColumnIds.map(colId => {
                                            ...
                                            return (
                                                <td key={colId} className={tdClassName} style={{ textAlign: isCentered ? 'center' : 'left', padding: cellPadding, fontWeight: colId === 'kecamatan' ? 600 : 400, color: colId === 'kecamatan' ? '#334155' : 'inherit' }}>
                ...
                {!loading && (
                    <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: '1rem', marginTop: '1rem', paddingTop: '1rem', borderTop: '1px solid var(--border)' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                            ...
                            <span style={{ fontSize: '0.875rem', color: 'var(--text-light)' }}>per halaman. Total: <strong style={{ color: 'var(--text)' }}>{total}</strong> kecamatan</span>
                        </div>
                    ...
                    <div style={{ background: 'white', borderRadius: '20px', width: '100%', maxWidth: '600px', maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.25)' }}>
                        ...
                        <form onSubmit={handleSubmit} style={{ padding: '2rem' }}>
                            <div className="form-group">
                                <label style={{ display: 'block', marginBottom: '0.5rem', fontWeight: 600, color: '#64748b' }}>Kecamatan</label>
                                <input type="text" required className="input-field" placeholder="Nama kecamatan" value={formData.kecamatan} onChange={(e) => setFormData({ ...formData, kecamatan: e.target.value })} style={{ width: '100%', padding: '0.75rem', borderRadius: '8px', border: '1px solid #e2e8f0' }} />
                            </div>
            ...
            <ConfirmModal isOpen={isConfirmOpen} onClose={() => setIsConfirmOpen(false)} onConfirm={handleConfirmDelete} title="Hapus Kecamatan?" message="Tindakan ini tidak dapat dibatalkan. Data kecamatan dan desa yang terkait mungkin terdampak." type="danger" />
        </div>
    );
}
