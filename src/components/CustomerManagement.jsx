...
// Removed hardcoded IMAGE_BASE_URL
...
export default function CustomerManagement({ user, canDo = () => true }) {
    ...
    return (
        <>
            <div style={{ animation: 'fadeIn 0.5s ease-out', display: 'flex', flexDirection: 'column', flex: 1, minHeight: 0 }}>
                ...
                <header className="header" style={{ marginBottom: isMobile ? '1.5rem' : '2rem' }}>
                    <div>
                        <h1 style={{ display: 'flex', alignItems: 'center', gap: isMobile ? '0.75rem' : '1rem', fontSize: isMobile ? '1.5rem' : '1.875rem', fontWeight: 800, color: '#0f172a', marginBottom: '0.25rem' }}>
                            <div style={{ background: 'rgba(14, 165, 233, 0.1)', padding: isMobile ? '0.5rem' : '0.625rem', borderRadius: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                <Droplets size={isMobile ? 24 : 32} color="var(--primary)" />
                            </div>
                            Data Pelanggan
                        </h1>
                        <p style={{ color: '#64748b', fontSize: isMobile ? '0.85rem' : '0.95rem', marginTop: '0.25rem' }}>Manajemen data pelanggan dan informasi meteran</p>
                    </div>
                    <div className="header-actions no-print" style={{ gap: isMobile ? '0.375rem' : '0.5rem' }}>
                        <button className="btn btn-outline" onClick={exportToCSV} title="Export CSV" style={{ height: isMobile ? '40px' : '42px', width: isMobile ? '40px' : '42px', padding: 0, borderRadius: '8px' }}>
                            <Download size={isMobile ? 16 : 18} />
                        </button>
                        <button className="btn btn-outline" onClick={exportToExcel} title="Export Excel" style={{ height: isMobile ? '40px' : '42px', width: isMobile ? '40px' : '42px', padding: 0, borderRadius: '8px' }}>
                                ...
                                <tbody>
                                    {currentItems.length > 0 ? currentItems.map((customer, index) => (
                                        <tr key={customer.id}>
                                            ...
                                            {(canDo('pelanggan', 'U') || canDo('pelanggan', 'D')) && (
                                                <td className="no-print sticky-col-right" style={{ textAlign: 'center', background: 'inherit', padding: '0.625rem 0.75rem' }}>
                                                    <div style={{ display: 'flex', gap: '0.35rem', justifyContent: 'center' }}>
                                                        {canDo('pelanggan', 'U') && (
                                                            <button className="btn btn-outline" style={{ padding: '0.25rem', width: '28px', height: '28px', borderRadius: '6px' }} onClick={() => handleOpenModal(customer)}>
                                                                <Edit2 size={14} />
                                                            </button>
                                                        )}
                                                        {canDo('pelanggan', 'D') && (
                                                            <button className="btn btn-outline" style={{ padding: '0.25rem', width: '28px', height: '28px', borderRadius: '6px', color: '#ef4444' }} onClick={() => handleDelete(customer.id)}>
                                                                <Trash2 size={14} />
                                                            </button>
                                                        )}
                                                    </div>
            ...
            <div onClick={e => e.stopPropagation()}>
                <SimpleImageViewer
                    ...
                    imageSrc={previewImage && previewImage.startsWith('/') ? `${IMAGE_BASE_URL}${previewImage}` : previewImage}
