...
import { Building2, Users, Droplets, BarChart2, History, FileText, X, LayoutDashboard, Settings, UserCircle, LogOut, Menu, ChevronLeft, Building, UserPlus, MapPin, ClipboardList, TrendingUp, HelpCircle, ChevronDown, FileSpreadsheet, FileDown, Table, Construction, Loader2 } from 'lucide-react';
...
/*
  Role naming convention (must match DB enum exactly - all lowercase):
  - superadmin  → full access
  - management  → read-only access
  - cabang      → branch operations
  - verificator → meter verification/recording
*/
...
export default function App() {
  ...
  const [expandedMenus, setExpandedMenus] = useState(['analisa']);
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        const res = await fetch('./api/login.php?action=check_session');
  ...
  const getPermissions = useCallback((role) => {
    ...
    return permissions[role.toLowerCase().trim()] || null;
  }, [permissions]);

  const canAccess = useCallback((itemId) => {
    if (!user) return false;
    if (user.role === 'superadmin') return true;
    const rolePerms = getPermissions(user.role);
    if (!rolePerms) return false;
    return !!rolePerms[itemId];
  }, [user, getPermissions]);

  const canDo = useCallback((itemId, action) => {
    if (!user) return false;
    if (user.role === 'superadmin') return true;
    // Current dynamic setup focuses on binary access (canAccess) 
    // for simplified initial dynamic transition
    return canAccess(itemId);
  }, [user, canAccess]);
  ...
  const filteredMenuItems = useMemo(() => {
    return menuItems
      ...
      .map(item => {
        if (item.subItems) {
          return { ...item, subItems: item.subItems.filter(sub => canAccess(sub.id)) };
        }
        return item;
      })
      .filter(item => {
        if (item.subItems) {
          return item.subItems.length > 0 || (item.id !== 'cabang_parent' && item.id !== 'laporan_parent');
        }
        return true;
      });
  ...
  return (
    <div className="app-layout">
      ...
      >
        <div className="sidebar-header" style={{ justifyContent: 'center', padding: (isSidebarOpen || isMobile) ? '1.25rem 1rem' : '1rem 0.5rem', transition: 'padding 0.3s ease' }}>
          <img
            src="logo.png"
            alt="PDAM App"
            style={{
        ...
        {(isSidebarOpen || isMobile) && user && (
          <div className="sidebar-user-profile">
            ...
            <div className="user-info">
              <div className="user-name">{user.nama_lengkap}</div>
        ...
        {!isSidebarOpen && !isMobile && user && (
          <div className="sidebar-user-profile-collapsed" title={`${user.nama_lengkap} (${user.role})`}>
        ...
        {!isMobile && (
          <div className="sidebar-footer">
            <button
              ...
              title={!isSidebarOpen ? "Keluar Akun" : ""}
            >
              <LogOut size={20} color="currentColor" />
              <span className="footer-label">Keluar Akun</span>
            </button>

            <button
              className="sidebar-toggle-btn"
              onClick={() => setIsSidebarOpen(!isSidebarOpen)}
              title={!isSidebarOpen ? "Buka Sidebar" : ""}
            >
              {isSidebarOpen ? (
                <>
                  <Menu size={20} color="currentColor" style={{ width: '20px' }} />
                  <span className="footer-label" style={{ fontWeight: 500 }}>Tutup Sidebar</span>
                </>
              ) : (
      ...
      < main className="main-content" >
        <div className="content-container">
          {activeTab === 'dashboard' && <MemoizedDashboard user={user} canDo={canDo} onTabChange={handleTabClick} />}
          {activeTab === 'pelanggan' && <MemoizedCustomerManagement user={user} canDo={canDo} />}
          {activeTab === 'catat' && <MemoizedRecordingManagement user={user} canDo={canDo} />}
          {activeTab === 'history_catat' && <MemoizedRecordingManagement user={user} canDo={canDo} isHistory={true} />}
          {(activeTab === 'petugas' || activeTab === 'laporan_petugas') && <MemoizedOfficerManagement user={user} canDo={canDo} isReport={activeTab === 'laporan_petugas'} onReportClose={() => setActiveTab('petugas')} />}
          {activeTab === 'petugas_mapping' && <MemoizedOfficerMapping user={user} canDo={canDo} />}
          {(activeTab === 'analisa' || activeTab === 'analisa_all') && <MemoizedMeterAnalysis user={user} canDo={canDo} key="analisa_all" />}
          {activeTab === 'analisa_review' && <MemoizedMeterAnalysis user={user} canDo={canDo} key="analisa_review" ocrStatusFilter="REVIEW" />}
          {activeTab === 'pelanggan_mapping' && <MemoizedCustomerMapping user={user} canDo={canDo} />}
          {(activeTab === 'cabang' || activeTab === 'laporan_cabang') && <MemoizedBranchManagement user={user} canDo={canDo} isReport={activeTab === 'laporan_cabang'} onReportClose={() => setActiveTab('cabang')} />}
          {activeTab === 'pengaturan' && <MemoizedSettings user={user} canDo={canDo} />}
        </div>
