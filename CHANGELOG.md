...
### Added
- **Dynamic RBAC System**: Migrated hardcoded permissions to a database-driven model (`sys_permissions` table).
- **Interactive Access Matrix**: Developed a new interactive table in Settings for real-time permission management.
- **Dynamic API**: Created `api/permissions.php` to handle role-based feature access and updates.

### Changed
- **Activity Log Optimization**: Implemented a compact UI with a 5-item pagination limit and scrollable timeline view to prevent page over-extension.
- **Sidebar & Logo Refinement**: Adjusted sidebar logo sizing and typography for a cleaner, more professional header.
- **User Modal Sync**: Updated user edit preview to reflect the current live permissions from the database.

### Fixed
- **App.jsx Initialization**: Corrected `ReferenceError` for missing React hooks and streamlined the authentication-to-permission fetching flow.

## [1.5.0] - 2026-03-04

### Added
- **AI OCR Integration**: Fully functional meter reading recognition using EasyOCR engine with automatic digit detection.
- **Hugging Face Deployment**: Migrated AI backend to Hugging Face Spaces (`abdinegara-sicater-ocr`) to overcome server resource limitations and ensure high availability.
- **AI-Validation Workflow**: Interactive validation button in recording forms with real-time similarity scoring and visual feedback (Match/Mismatch badges).
- **PHP 7 Compatibility**: Added polyfills for `str_starts_with` and `str_ends_with` in `api/config.php` to ensure backend stability on older server environments.

### Fixed
- **Build System Synchronization**: Resolved a critical issue in the root `index.html` that prevented production assets from correctly bundling the latest source changes.
- **AI Connection Error**: Optimized the bridge between the production domain and the AI service, resolving the "Koneksi Terputus" error.
