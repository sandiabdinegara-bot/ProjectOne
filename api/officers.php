...
switch ($method) {
    case 'GET':
        ...
        foreach ($rows as &$row) {
            if (!empty($row['foto_petugas']) && !str_starts_with($row['foto_petugas'], 'data:image')) {
                $filename = basename(str_replace(['/', '\\'], '/', $row['foto_petugas']));
                $filename = str_replace(' ', '_', $filename);
                $filename = findFileCaseInsensitive($upload_dir, $filename);
    ...
    case 'DELETE':
        if (isset($_GET['id'])) {
            ...
            try {
                ...
                sendError("Gagal menghapus data petugas.", $e->getMessage());
            }
        }
        break;
}
