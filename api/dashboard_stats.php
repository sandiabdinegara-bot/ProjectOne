...
try {
    ...
    $ocr_distribution = array_map(function($item) {
        return [
            'label' => $item['label'] ?? 'Unknown',
            'value' => (int) $item['value']
        ];
    }, $stmt_ocr_status->fetchAll(PDO::FETCH_ASSOC));
    ...
    $kondisi_distribution = array_map(function($item) {
        return [
            'label' => $item['label'] ?? 'Normal',
            'value' => (int) $item['value']
        ];
    }, $stmt_kondisi->fetchAll(PDO::FETCH_ASSOC));
    ...
    sendError("Gagal memuat statistik dashboard.", $e->getMessage());
}
?>